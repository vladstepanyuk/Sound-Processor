...
soundProcessor::ProgramContext::ProgramContext(int argc, char **argv) {
  ...
  else if (!vm.count("input") || !vm.count("output")) throw std::runtime_error("no input or output files");

  auto *configFile = new std::ifstream();
  configFile->open(configFileName);
  if (!configFile->is_open()) throw std::runtime_error("can't open config file");
  configStream = configFile;

  auto *outputFile = new std::ofstream();
  outputFile->open(outputFileName, std::ios_base::binary);
  if (!outputFile->is_open()) throw std::runtime_error("can't open output file");
  ...
  for (int i = 0; i < inputFilesNames.size(); ++i) {
    ...
    if (!inputFile->is_open()) throw std::runtime_error("can't open input file");
...
std::istream *soundProcessor::ProgramContext::returnInputStream(int N) {
  if (N >= inputStreams.size()) throw std::runtime_error("out of bound");
