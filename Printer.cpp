...
void Printer::printNBytesFromIntoOut(std::istream &inputStream, std::ostream &outStream, int N) {
  ...
  for (int i = 0; i < N / StandardStringBuffer; ++i) {
    ...
    if (!outStream.write(&str[0], StandardStringBuffer).good()) throw std::runtime_error("cant write");
  }
  if (!inputStream.read(&str[0], N % StandardStringBuffer).good()) throw std::runtime_error("cant reed");
  if (!outStream.write(&str[0], N % StandardStringBuffer).good()) throw std::runtime_error("cant write");
}
...
void Printer::printNSamples(std::ostream &outputStream, const std::vector<short int> &sampleBuff, int N) {
  ...
  for (int i = 0; i < N; ++i) {
    static auto one = static_cast<unsigned long long>(oneByte);
    for (int j = 0; j < ShortIntSize; ++j)
      stringBuff[i * ShortIntSize + j] = (sampleBuff[i] & (one << BitsPerByte * (j))) >> BitsPerByte * j;
  }
  if (outputStream.write(&stringBuff[0], N * ShortIntSize).bad()) throw std::runtime_error("can't write");
}
