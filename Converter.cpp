...
MuteConverter::convert(std::vector<int> args, ProgramContext &context, std::istream &prevRes, std::ostream &res) {
  ...
  if (args.size() < MuteArgc) throw std::runtime_error("wrong args number");

  if (args[MuteArgcFirstSecond] > args[MuteArgcLastSecond]) throw std::runtime_error("wrong format config");
...
MixConverter::convert(std::vector<int> args, ProgramContext &context, std::istream &prevRes, std::ostream &res) {
  ...
  if (args.size() < MixArgc) throw std::runtime_error("wrong args number");
  ...
  if (args[MixArgcFirstSecond] * StandardHz * BytesPerSample > minData) throw std::runtime_error("wrong format");
...
MultConverter::convert(std::vector<int> args, ProgramContext &context, std::istream &prevRes, std::ostream &res) {
  ...
  if (args.size() < MultArgc) throw std::runtime_error("wrong args number");

  if (args[MultArgcFirstSecond] > args[MultArgcLastSecond]) throw std::runtime_error("wrong format config");
  ...
  if (args[MultArgcLastSecond] * StandardHz * BytesPerSample > dataSize) throw std::runtime_error("wrong format");
  ...
  if ((args[MultArgcLastSecond] - args[MuteArgcFirstSecond]) * StandardHz % SampleBuffSize != 0) {
    buffSample = reader.readNSamples(prevRes,
                                     (args[MultArgcLastSecond] - args[MuteArgcFirstSecond]) * StandardHz
                                         % SampleBuffSize);
    for (int j = 0; j < (args[MultArgcLastSecond] - args[MuteArgcFirstSecond]) * StandardHz % SampleBuffSize; ++j)
      buffSample[j] *= args[MultArgcMultCoef];
    printer.printNSamples(res,
                          buffSample,
                          (args[MultArgcLastSecond] - args[MuteArgcFirstSecond]) * StandardHz % SampleBuffSize);
  }
